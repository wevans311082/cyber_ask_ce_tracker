...
class ClientAssessmentDetailView(ClientRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        return Assessment.objects.filter(client=profile.client).prefetch_related(
            ...
            'date_options__proposed_by', # Prefetch date options
        ).select_related('client', 'assessor__userprofile') # Select related assessor profile
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ce_plus_sample_items'] = sorted(sample_items_with_status, key=lambda x: (x.item_type, str(x.operating_system or '')))


        # --- Scope Summary Logic ---
        scope_summary_data = defaultdict(lambda: {'count': 0, 'os_types': defaultdict(lambda: {'count': 0, 'is_supported': True, 'is_eol': False})})
        ...
        for item in all_scope_items:
            ...
            if item.operating_system:
                os_name_str, vendor_hint_str = str(item.operating_system), item.operating_system.vendor.lower() if item.operating_system.vendor else "unknown"
            ...
            cat_map = {'Server': 'servers', 'Laptop': 'workstations', 'Desktop': 'workstations', 'Mobile': 'mobiles', 'Firewall': 'network_devices', 'Router': 'network_devices', 'Switch': 'network_devices', 'IP': 'network_devices', 'SaaS': 'cloud_services', 'PaaS': 'cloud_services', 'IaaS': 'cloud_services'}
        ...
        for category, data in scope_summary_data.items():
             if category not in ['total_items', 'has_unsupported_or_eol']: final_scope_summary[category] = {'count': data['count'], 'os_types': {key: dict(val) for key, val in data['os_types'].items()}}
        ...
        # Determine if date management actions (Prefer, Delete Own Suggested) should be enabled
        ...
        # Always provide the form instance for proposing dates
        context['propose_date_form'] = AssessmentDateOptionForm(assessment=assessment)

        # Assessor unavailable dates (for date picker disabling)
        ...
        # CE+ Window Info (for display text)
        ...
        if assessment.date_target_end:
             try:
                 # Use django_timezone consistently
